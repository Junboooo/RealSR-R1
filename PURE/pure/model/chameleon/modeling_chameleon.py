...
class ChameleonModel(ChameleonPreTrainedModel):
    ...
    def __init__(self, config: ChameleonConfig):
        ...
        # 添加condition layers
        # self.layer_internal = config.num_hidden_layers // 4
        # self.condition_layers = torch.nn.ModuleList()
        # for layer_id in range(config.num_hidden_layers):
        #     self.condition_layers.append(MLP(config.hidden_size,config.hidden_size,config.hidden_size))
...
class ChameleonForConditionalGeneration(ChameleonPreTrainedModel):
    ...
    def apply_lora(self, lora_config):
        ...
        self.lm_head = get_peft_model(self.lm_head, lora_config)
