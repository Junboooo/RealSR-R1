...
print(os.getcwd())
import sys
sys.path.append(os.path.abspath('/work/docker/GRPO/PURE-main/'))
...
class FlexARItemProcessor(MMConvItemProcessor):
    ...
    def __init__(
        self,
        tokenizer="/work/docker/GRPO/PURE-main/pure/ckpts/Alpha-VLLM/PRUE",
        ...
        self.chameleon_ori_vocab = chameleon_vae_ori.VocabInfo(
            json.load(open("/work/docker/GRPO/PURE-main/pure/tokenizer/text_tokenizer.json", encoding="utf8"))["model"]["vocab"]
        )
        ...
        self.chameleon_ori_image_tokenizer = chameleon_vae_ori.ImageTokenizer(
            ckpt_path="/work/docker/GRPO/PURE-main/pure/tokenizer/vq_ds8_c2i.pt",
            device="cuda",
        )
        self.count=0
    @staticmethod
    ...
    def process_item(self, item, training_mode=False, out_flatten=True):
        ...
        if training_mode:
            ...
            ##
            # ids=torch.nonzero(torch.tensor([token==16604 for token in tokens]))+1
            # print([len(tokens[idd]['input_ids']) for idd in ids])
            # print([tokens[idd]['input_ids'][:3] for idd in ids])
            #
            input_tokens_item = []
    ...
    def decode_image(self, tokens: List[int]) -> Image.Image:
        ...
        h_latent_dim, w_latent_dim = h_grids * 4, w_grids * 4
